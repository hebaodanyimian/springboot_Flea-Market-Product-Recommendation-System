	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="''">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/friend?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">表名</div>
						<div class="text "  >{{detail.tablename}}</div>
					</div>
					<div class="item">
						<div class="lable">别名</div>
						<div class="text "  >{{detail.alias}}</div>
					</div>
					<div class="item">
						<div class="lable">类型(0:好友申请，1:好友，2:消息)</div>
						<div class="text "  >{{detail.type}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('friend','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('friend','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/friend', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 删除
			async delClick(){
				await this.$confirm('是否删除此好友表？') .then(_ => {
					this.$http.post('friend/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
