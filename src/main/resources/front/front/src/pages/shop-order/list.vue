<template>
	<div :style='{"padding":"0 calc((100% - 1200px)/2) 20px","margin":"0px auto","color":"#666","background":"#fff","width":"100%","fontSize":"16px","position":"relative"}'>
		...
		<div class="section-title" :style='{"padding":"0 0 0 50px","margin":"0px auto 0","color":"rgb(25, 190, 212)","textAlign":"left","background":"url(http://codegen.caihongy.cn/20250220/e85584abac2c4b3ebee6d1c211618e75.png) no-repeat center bottom","width":"100%","fontSize":"26px","lineHeight":"100px","position":"relative"}'>我的订单</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getMyOrderList(page) {
				this.$http.get('orders/list', {params: {page, limit: this.pageSize,sort:'addtime',order:'desc', userid: Number(localStorage.getItem('frontUserid')), status: this.activeName}}).then(res => {
					if (res.data.code == 0) {
