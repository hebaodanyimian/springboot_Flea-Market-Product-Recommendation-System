	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="金额" prop="amount" >
					<el-input-number v-model="ruleForm.amount" placeholder="金额" :disabled="ro.amount"></el-input-number>
				</el-form-item>
				<el-form-item v-else class="input" label="金额" prop="amount" >
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `chargerecord/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.chargerecordCrossAddOrUpdateFlag = false;
											this.parent.search();
